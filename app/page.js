...
export default function AuditSEOLocal() {
  ...
  const [locations, setLocations] = useState([{ id: 1, placeId: '', name: '', city: '', lat: '', lon: '', loading: false, error: '' }]);
  ...
  const addLocation = () => {
    setLocations([...locations, { id: Date.now(), placeId: '', name: '', city: '', lat: '', lon: '', loading: false, error: '' }]);
  };
  ...
  // NOUVELLE FONCTION : Récupérer les infos du lieu via Place ID
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.address) {
          ...
          if (parts.length >= 2) {
            // Prendre l'avant-dernière partie (souvent la ville + code postal)
  ...
  const styles = {
    app: { minHeight: '100vh', background: 'linear-gradient(135deg, #0a1628 0%, #0d1f3c 50%, #0a1628 100%)', color: '#fff', fontFamily: "'Segoe UI', -apple-system, sans-serif" },
    header: { borderBottom: '1px solid rgba(255,255,255,0.08)', background: 'rgba(10, 22, 40, 0.9)', padding: '16px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' },
    logo: { display: 'flex', alignItems: 'center', gap: '12px' },
    logoIcon: { width: '40px', height: '40px', background: 'linear-gradient(135deg, #10b981, #06b6d4)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px' },
    container: { maxWidth: '1200px', margin: '0 auto', padding: '40px 24px' },
    card: { background: 'rgba(26, 41, 66, 0.8)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '28px', marginBottom: '24px' },
    btn: { display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '14px 28px', borderRadius: '10px', fontSize: '14px', fontWeight: 600, cursor: 'pointer', border: 'none' },
    btnPrimary: { background: 'linear-gradient(135deg, #10b981, #06b6d4)', color: '#fff' },
    btnSecondary: { background: 'rgba(255,255,255,0.05)', color: '#fff', border: '1px solid rgba(255,255,255,0.1)' },
    input: { width: '100%', padding: '12px 16px', background: '#1a2942', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#fff', fontSize: '14px', fontFamily: 'monospace', boxSizing: 'border-box' },
    label: { display: 'block', fontSize: '12px', fontWeight: 600, color: '#06b6d4', marginBottom: '8px', textTransform: 'uppercase', letterSpacing: '0.5px' },
    statsGrid: { display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '32px' },
    statCard: { background: 'rgba(26, 41, 66, 0.8)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '24px', textAlign: 'center' },
  };
      ...
      <div style={styles.app}>
        ...
        <div style={styles.container}>
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '8px 20px', background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.3)', borderRadius: '100px', fontSize: '12px', fontWeight: 600, color: '#10b981', marginBottom: '20px' }}>⚡ Audit automatisé</div>
            <h2 style={{ fontSize: '42px', fontWeight: 800, marginBottom: '16px', lineHeight: 1.1 }}>Analysez votre visibilité<br /><span style={{ color: '#10b981' }}>Google Maps</span></h2>
            <p style={{ fontSize: '18px', color: '#94a3b8', maxWidth: '600px', margin: '0 auto 40px' }}>Obtenez un audit SEO local complet avec estimation de l'impact financier</p>
          </div>
          <div style={{ ...styles.card, maxWidth: '700px', margin: '0 auto' }}>
            <div style={{ background: 'linear-gradient(135deg, rgba(59, 130, 246, 0.1), rgba(6, 182, 212, 0.1))', border: '1px solid rgba(59, 130, 246, 0.2)', borderRadius: '12px', padding: '20px', marginBottom: '24px' }}>
              <h3 style={{ fontSize: '15px', fontWeight: 600, marginBottom: '12px' }}>🔑 Comment obtenir une clé API SerpAPI ?</h3>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <input type="password" style={styles.input} placeholder="Collez votre clé API ici..." value={apiKey} onChange={(e) => setApiKey(e.target.value)} />
            </div>
            <button style={{ ...styles.btn, ...styles.btnPrimary, width: '100%' }} onClick={() => apiKey ? setStep(2) : alert('Veuillez entrer une clé API')}>Continuer →</button>
          </div>
        </div>
      </div>
    );
  ...
  if (step === 2) {
    return (
      <div style={styles.app}>
        ...
        <div style={styles.container}>
          <h2 style={{ fontSize: '28px', fontWeight: 700, marginBottom: '32px', textAlign: 'center' }}>Configuration de l'audit</h2>
            ...
            <div style={{ marginTop: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '8px' }}>
                {Object.entries(SECTOR_CONFIG).map(([key, val]) => (
                  <button key={key} onClick={() => setSector(key)} style={{ padding: '12px', background: sector === key ? 'rgba(16, 185, 129, 0.2)' : 'rgba(255,255,255,0.05)', border: sector === key ? '2px solid #10b981' : '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#fff', fontSize: '12px', cursor: 'pointer' }}>{val.name}</button>
                ))}
              </div>
            </div>
          </div>
          ...
          <div style={styles.card}>
            ...
            <div style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.3)', borderRadius: '10px', padding: '16px', marginBottom: '20px' }}>
              <p style={{ fontSize: '13px', color: '#10b981', margin: 0 }}>
                <strong>✨ Nouveau :</strong> Entrez juste le Place ID et cliquez sur "Récupérer les infos" - les coordonnées et le nom seront remplis automatiquement !
              </p>
            </div>
            <div style={{ background: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.3)', borderRadius: '10px', padding: '16px', marginBottom: '20px' }}>
              <h4 style={{ fontSize: '13px', fontWeight: 600, color: '#f59e0b', marginBottom: '10px' }}>🗺️ Comment trouver un Place ID ?</h4>
              <ol style={{ fontSize: '12px', color: '#94a3b8', lineHeight: 1.8, paddingLeft: '18px', margin: 0 }}>
                <li>Allez sur <a href="https://developers.google.com/maps/documentation/javascript/examples/places-placeid-finder" target="_blank" rel="noreferrer" style={{ color: '#10b981' }}>Place ID Finder Google</a></li>
                <li>Cherchez votre établissement</li>
                <li>Copiez le Place ID affiché</li>
              </ol>
            </div>

            {locations.map((loc, i) => (
              <div key={loc.id} style={{ background: '#1a2942', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '16px', marginBottom: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                  <span style={{ fontWeight: 600 }}>Établissement {i + 1}</span>
                  {locations.length > 1 && (
                    <button onClick={() => removeLocation(loc.id)} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', fontSize: '12px' }}>Supprimer</button>
                  )}
                </div>
                ...
                {/* Message d'erreur */}
                {loc.error && (
                  <div style={{ color: '#ef4444', fontSize: '12px', marginBottom: '12px' }}>❌ {loc.error}</div>
                )}
                ...
                {loc.name && (
                  <div style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.2)', borderRadius: '8px', padding: '12px', marginBottom: '12px' }}>
                    <div style={{ fontSize: '14px', fontWeight: 600, color: '#10b981' }}>✅ {loc.name}</div>
                    {loc.rating && <div style={{ fontSize: '12px', color: '#94a3b8', marginTop: '4px' }}>⭐ {loc.rating} ({loc.reviews} avis)</div>}
                  </div>
                )}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  <div>
                    <label style={{ ...styles.label, fontSize: '10px' }}>Nom (modifiable)</label>
                  ...
                  <div>
                    <label style={{ ...styles.label, fontSize: '10px' }}>Ville (pour les mots-clés)</label>
                ...
                {/* Coordonnées (lecture seule ou éditable) */}
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginTop: '10px' }}>
                  <input style={{ ...styles.input, background: '#0a1628', fontSize: '12px' }} placeholder="Latitude" value={loc.lat} onChange={(e) => updateLocation(loc.id, 'lat', e.target.value)} />
                  <input style={{ ...styles.input, background: '#0a1628', fontSize: '12px' }} placeholder="Longitude" value={loc.lon} onChange={(e) => updateLocation(loc.id, 'lon', e.target.value)} />
                </div>
              </div>
            ))}
            
            <button onClick={addLocation} style={{ width: '100%', padding: '14px', background: 'transparent', border: '2px dashed rgba(255,255,255,0.2)', borderRadius: '10px', color: '#64748b', cursor: 'pointer' }}>+ Ajouter un établissement</button>
          </div>
          ...
          <div style={styles.card}>
            ...
            {keywords.map((kw, i) => (
              <div key={i} style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
                <input style={{ ...styles.input, flex: 1 }} value={kw} onChange={(e) => updateKeyword(i, e.target.value)} />
                {keywords.length > 1 && (
            ...
            <button onClick={addKeyword} style={{ marginTop: '8px', padding: '10px 16px', background: 'rgba(16,185,129,0.1)', border: '1px dashed #10b981', borderRadius: '8px', color: '#10b981', cursor: 'pointer' }}>+ Ajouter un mot-clé</button>
          </div>

          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <button style={{ ...styles.btn, ...styles.btnSecondary }} onClick={() => setStep(1)}>← Retour</button>
            <button style={{ ...styles.btn, ...styles.btnPrimary }} onClick={runAnalysis} disabled={!businessName || !activity || locations.every(l => !l.placeId || !l.lat)}>🚀 Lancer l'analyse</button>
          </div>
        </div>
      </div>
    );
      ...
      <div style={styles.app}>
        ...
        <div style={styles.container}>
          <div style={{ ...styles.card, maxWidth: '500px', margin: '80px auto', textAlign: 'center' }}>
            <div style={{ width: '120px', height: '120px', margin: '0 auto 32px', borderRadius: '50%', background: `conic-gradient(#10b981 ${progress * 3.6}deg, #1a2942 0deg)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ width: '100px', height: '100px', borderRadius: '50%', background: '#0a1628', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px', fontWeight: 700 }}>{progress}%</div>
            </div>
    ...
    return (
      <div style={styles.app}>
        <header style={styles.header}>
          ...
          <button style={{ ...styles.btn, ...styles.btnSecondary, padding: '10px 20px' }} onClick={() => { setStep(1); setResults(null); }}>← Nouvel audit</button>
        </header>

        <div style={styles.container}>
          <div style={{ textAlign: 'center', marginBottom: '40px' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '6px 16px', background: 'rgba(59, 130, 246, 0.1)', border: '1px solid rgba(59, 130, 246, 0.3)', borderRadius: '20px', fontSize: '11px', color: '#3b82f6', marginBottom: '16px' }}>📊 Audit SEO Local 2025</div>
            <h2 style={{ fontSize: '36px', fontWeight: 800, marginBottom: '8px' }}>Audit SEO Local<br /><span style={{ color: '#10b981' }}>{results.business}</span></h2>
          ...
          <div style={styles.card}>
            <div style={{ fontSize: '13px', fontWeight: 700, color: '#ef4444', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '16px' }}>📊 Répartition des pertes par site</div>
            {results.locations.sort((a, b) => b.estimatedLoss - a.estimatedLoss).map((loc, i) => (
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', background: '#1a2942', borderRadius: '8px', marginBottom: '8px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '20px', marginBottom: '32px' }}>
            {results.locations.map((loc, i) => (
              <div key={i} style={styles.card}>
                ...
                <div style={{ fontSize: '10px', fontWeight: 700, color: '#06b6d4', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '10px' }}>Positions Clés</div>
                {loc.rankings.map((r, j) => (
                  <div key={j} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: '#1a2942', borderRadius: '6px', marginBottom: '6px' }}>
          ...
          <div style={{ ...styles.card, overflowX: 'auto' }}>
            <h3 style={{ fontSize: '16px', fontWeight: 700, marginBottom: '20px' }}>Matrice de Positionnement par Requête</h3>
